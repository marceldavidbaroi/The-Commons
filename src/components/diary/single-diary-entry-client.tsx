...
export function SingleDiaryEntryClient({ targetId }: { targetId: string }) {
  ...
  const combinedEntries = storeEntries.length > 0 ? storeEntries : allFetchedEntries;


  // Locate current entry and its parent diary
  const currentEntry = useMemo(() => {
    const found = combinedEntries.find((e: DiaryEntry) => e.id === targetId || String(e.pageNumber) === targetId);
    if (found) return found;
    return getEntryById(targetId) || combinedEntries[0] || allFetchedEntries[0];
  }, [targetId, getEntryById, combinedEntries, allFetchedEntries]);

  const currentDiary = useMemo(() => {
    if (currentEntry) {
      const found = diaries.find((d: Diary) => d.id === currentEntry.diaryId) || getDiaryById(currentEntry.diaryId);
      if (found) return found;
    }
    const directDiary = diaries.find((d: Diary) => d.id === targetId) || getDiaryById(targetId);
  ...
  const diaryEntries = useMemo(() => {
    ...
    return combinedEntries.filter((e: DiaryEntry) => e.diaryId === currentDiary.id);
